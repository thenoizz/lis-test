...
echo "Covers FCOPY_non_ascii" > $summaryLog
...
# Check to see Linux VM is running FCOPY backup daemon 
$sts = RunRemoteScript "Check_FCOPY_Daemon.sh"
if (-not $sts[-1])
{
...
Remove-Item -Path "Check_FCOPY_Daemon.sh.log" -Force
...
if ($gsi.OperationalStatus -ne "OK") {
    ...
    1..400000 | % {
        $nonAsciiChars>>$pathToFile
    }

    #Checking if file was successfully created
    if (-not $?){
        Write-Output "ERROR: Unable to create the file" >> $summaryLog
        $retVal = $False   
    }
    else {
        Write-Output"File $testfile successfully created"
    }
...
{ 
    Write-Output "ERROR: MD5 checksums are not matching"
    Remove-Item -Path "FCOPY_non_ascii.sh.log" -Force
    return $False
} 
else 
{ 
    Write-Output "MD5 checksums are matching" >> $summaryLog
    Remove-Item -Path "FCOPY_non_ascii.sh.log" -Force
}

# Removing the temporary test file
Write-Output "PATH : $pathToFile"
...
if ($? -ne "True") {
    ...
    Write-Output "INFO: FCOPY_non_ascii: Success"
    $results = "Passed"
    $retVal = $True
}
